...
from model import GATBasedMolecularGraphNeuralNetworkV3, GATBasedMolecularGraphNeuralNetwork194
...
wandb.init(project="belka", settings=wandb.Settings(code_dir="./systematic/mega_ml"), tags=['GIN_fance','full_split'])
...
train_dataset = FileMegaSet(TRAIN_FILE, ratio=1)
...
# Model
model = GATBasedMolecularGraphNeuralNetwork194().to(device)
model.load_state_dict(torch.load('/home/anonymous/belka/systematic/mega_ml/models/tmp/277-e15.pth'), strict=False)

optimizer = Ranger21(model.parameters(), lr = 0.0003, num_epochs=10, num_batches_per_epoch=24028)
